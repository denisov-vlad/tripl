...
async def list_variables(session: SessionDep, slug: str):
    return await variable_service.list_variables(session, slug)


@router.post("", response_model=VariableResponse, status_code=201)
async def create_variable(session: SessionDep, slug: str, data: VariableCreate):
...
async def delete_variable(session: SessionDep, slug: str, variable_id: uuid.UUID):
